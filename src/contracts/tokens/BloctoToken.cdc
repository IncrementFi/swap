...
access(all) contract BloctoToken: FungibleToken {
  // Frozen flag controlled by Admin
  access(all) var isFrozen: Bool

  // Total supply of BloctoTokens in existence
  ...
  //
  access(all) fun createEmptyVault(vaultType: Type): @BloctoToken.Vault {
    return <- create Vault(balance: 0.0)
  }
  ...
  access(all) resource Administrator {
    ...
    access(all) fun freeze() {
      BloctoToken.isFrozen = true
    }

    access(all) fun unfreeze() {
      BloctoToken.isFrozen = false
    }
  ...
  access(all) resource interface TeleportControl {
    access(all) fun teleportIn(amount: UFix64, from: [UInt8], hash: String): @BloctoToken.Vault
  ...
  access(all) resource TeleportAdmin: TeleportUser, TeleportControl {
    ...
    access(all) let feeCollector: @BloctoToken.Vault
    ...
    access(all) fun teleportIn(amount: UFix64, from: [UInt8], hash: String): @BloctoToken.Vault {
      pre {
        !BloctoToken.isFrozen: "Teleport service is frozen"
        ...
        !(BloctoToken.teleported[hash] ?? false): "Same hash already teleported"
      }
      BloctoToken.totalSupply = BloctoToken.totalSupply + amount
      self.allowedAmount = self.allowedAmount - amount

      BloctoToken.teleported[hash] = true
    ...
    init(allowedAmount: UFix64) {
      ...
      self.feeCollector <- BloctoToken.createEmptyVault(vaultType: Type<@BloctoToken.Vault>())
      self.inwardFee = 0.01
      self.outwardFee = 3.0
