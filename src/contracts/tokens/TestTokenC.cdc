...
access(all) contract TestTokenC: FungibleToken {
  ...
  // Total supply of testTokenC in existence
  ...
  access(all) resource Vault: FungibleToken.Vault {
    ...
    access(contract) fun burnCallback() {
      if self.balance > 0.0 {
        TestTokenC.totalSupply = TestTokenC.totalSupply - self.balance
      }
      self.balance = 0.0
    }
    ...
    access(all) fun deposit(from: @{FungibleToken.Vault}) {
      let vault <- from as! @TestTokenC.Vault
  ...
  //
  access(all) fun createEmptyVault(vaultType: Type): @TestTokenC.Vault {
    return <-create Vault(balance: 0.0)
  }
  ...
  access(all) resource Minter {
    ...
    //
    access(all) fun mintTokens(amount: UFix64): @TestTokenC.Vault {
      pre {
          amount > 0.0: "Amount minted must be greater than zero"
      }
      TestTokenC.totalSupply = TestTokenC.totalSupply + amount
  ...
  access(all) resource Administrator {
    ...
    // transactions/testTokenC/minter/setup_testTokenC_minter.cdc
    // then the admin account running:
    // transactions/testTokenC/admin/deposit_testTokenC_minter.cdc
    //
  ...
  // TODO delete
  access(all) var test_minter:@Minter

  init() {
    self.AdminStoragePath = /storage/testTokenCAdmin
    self.MinterProxyPublicPath = /public/testTokenCMinterProxy
    self.MinterProxyStoragePath = /storage/testTokenCMinterProxy
