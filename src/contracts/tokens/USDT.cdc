...
access(all) contract USDT: FungibleToken {
  ...
  // Total supply of usdt in existence
  ...
  access(all) resource Vault: FungibleToken.Vault {
    ...
    access(contract) fun burnCallback() {
      if self.balance > 0.0 {
        USDT.totalSupply = USDT.totalSupply - self.balance
      }
      self.balance = 0.0
    }
  ...
  //
  access(all) fun createEmptyVault(vaultType: Type): @USDT.Vault {
    return <-create Vault(balance: 0.0)
  }
  ...
  access(all) resource Minter {
    ...
    //
    access(all) fun mintTokens(amount: UFix64): @USDT.Vault {
      pre {
          amount > 0.0: "Amount minted must be greater than zero"
      }
      USDT.totalSupply = USDT.totalSupply + amount
  ...
  access(all) resource Administrator {
    ...
    // transactions/usdt/minter/setup_usdt_minter.cdc
    // then the admin account running:
    // transactions/usdt/admin/deposit_usdt_minter.cdc
    //
  ...
  init() {
    self.AdminStoragePath = /storage/usdtAdmin
    self.MinterProxyPublicPath = /public/usdtMinterProxy
    self.MinterProxyStoragePath = /storage/usdtMinterProxy
