...
access(all) contract LogEntry {
    ...
    access(all) event SweepTokensToFlow(
        tokensToSweep: [String],
        amountsToSweep: [UFix64],
        flowTokensOut: [UFix64]
    )
    // Log swap lp migration from v1 to v2
    access(all) event MigrateSwapLpFromV1ToV2(
        token0Key: String,
        token1Key: String,
        lpToRemove: UFix64,
    ...
    access(all) fun LogSweepTokensToFlow(tokensToSweep: [String], amountsToSweep: [UFix64], flowTokensOut: [UFix64]) {
        emit SweepTokensToFlow(tokensToSweep: tokensToSweep, amountsToSweep: amountsToSweep, flowTokensOut: flowTokensOut)
    }
    access(all) fun LogMigrateSwapLpFromV1ToV2(token0Key: String, token1Key: String, lpToRemove: UFix64, token0WithdrawFromV1: UFix64, token1WithdrawFromV1: UFix64, token0LeftAfterFirstAddV2Lp: UFix64, token1LeftAfterFirstAddV2Lp: UFix64, zappedAmount: UFix64) {
        emit MigrateSwapLpFromV1ToV2(token0Key: token0Key, token1Key: token1Key, lpToRemove: lpToRemove, token0WithdrawFromV1: token0WithdrawFromV1, token1WithdrawFromV1: token1WithdrawFromV1, token0LeftAfterFirstAddV2Lp: token0LeftAfterFirstAddV2Lp, token1LeftAfterFirstAddV2Lp: token1LeftAfterFirstAddV2Lp, zappedAmount: zappedAmount)
    }
    access(all) fun LogBurnSwapLp(token0Key: String, token1Key: String, pairAddr: Address, amountToBurn: UFix64) {
        emit BurnSwapLp(token0Key: token0Key, token1Key: token1Key, pairAddr: pairAddr, amountToBurn: amountToBurn)
    }
    // ... More to be added here ...
}