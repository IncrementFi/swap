...
access(all) contract FlowSwapPair: FungibleToken {
  ...
  access(all) resource TokenBundle {
    access(all) var token1: @FlowToken.Vault
  ...
  // createTokenBundle
  //
  access(all) fun createTokenBundle(fromToken1: @FlowToken.Vault, fromToken2: @TeleportedTetherToken.Vault): @FlowSwapPair.TokenBundle {
  ...
  access(all) resource SwapProxy {
    access(all) fun swapToken1ForToken2(from: @FlowToken.Vault): @TeleportedTetherToken.Vault {
      return <- FlowSwapPair._swapToken1ForToken2(from: <-from)
    }

    access(all) fun swapToken2ForToken1(from: @TeleportedTetherToken.Vault): @FlowToken.Vault {
      return <- FlowSwapPair._swapToken2ForToken1(from: <-from)
    }

    access(all) fun addLiquidity(from: @FlowSwapPair.TokenBundle): @FlowSwapPair.Vault {
      return <- FlowSwapPair._addLiquidity(from: <-from)
    }

    access(all) fun removeLiquidity(from: @FlowSwapPair.Vault): @FlowSwapPair.TokenBundle {
  ...
  access(all) resource Admin {
    ...
    access(all) fun addInitialLiquidity(from: @FlowSwapPair.TokenBundle): @FlowSwapPair.Vault {
      pre {
        FlowSwapPair.totalSupply == 0.0: "Pair already initialized"
      }
    ...
    access(all) fun createSwapProxy(): @FlowSwapPair.SwapProxy {
  ...
  access(all) fun donateLiquidity(from: @FlowSwapPair.TokenBundle) {
    let token1Vault <- from.withdrawToken1()
    let token2Vault <- from.withdrawToken2()
