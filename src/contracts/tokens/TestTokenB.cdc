...
access(all) contract TestTokenB: FungibleToken {
  ...
  // Total supply of testTokenB in existence
  ...
  access(all) resource Vault: FungibleToken.Vault {
    ...
    access(contract) fun burnCallback() {
      if self.balance > 0.0 {
        TestTokenB.totalSupply = TestTokenB.totalSupply - self.balance
      }
      self.balance = 0.0
    }
    ...
    access(all) fun deposit(from: @{FungibleToken.Vault}) {
      let vault <- from as! @TestTokenB.Vault
  ...
  //
  access(all) fun createEmptyVault(vaultType: Type): @TestTokenB.Vault {
    return <-create Vault(balance: 0.0)
  }
  ...
  access(all) resource Minter {
    ...
    //
    access(all) fun mintTokens(amount: UFix64): @TestTokenB.Vault {
      pre {
          amount > 0.0: "Amount minted must be greater than zero"
      }
      TestTokenB.totalSupply = TestTokenB.totalSupply + amount
  ...
  access(all) resource Administrator {
    ...
    // transactions/testTokenB/minter/setup_testTokenB_minter.cdc
    // then the admin account running:
    // transactions/testTokenB/admin/deposit_testTokenB_minter.cdc
    //
  ...
  init() {
    self.AdminStoragePath = /storage/testTokenBAdmin
    self.MinterProxyPublicPath = /public/testTokenBMinterProxy
    self.MinterProxyStoragePath = /storage/testTokenBMinterProxy
