...
access(all) contract TestTokenA: FungibleToken {
  ...
  // Total supply of testTokenA in existence
  ...
  access(all) resource Vault: FungibleToken.Vault {
    ...
    access(contract) fun burnCallback() {
      if self.balance > 0.0 {
        TestTokenA.totalSupply = TestTokenA.totalSupply - self.balance
      }
      self.balance = 0.0
    }
    ...
    access(all) fun deposit(from: @{FungibleToken.Vault}) {
      let vault <- from as! @TestTokenA.Vault
  ...
  //
  access(all) fun createEmptyVault(vaultType: Type): @TestTokenA.Vault {
    return <-create Vault(balance: 0.0)
  }
  ...
  access(all) resource Minter {
    ...
    //
    access(all) fun mintTokens(amount: UFix64): @TestTokenA.Vault {
        pre {
            amount > 0.0: "Amount minted must be greater than zero"
        }
        TestTokenA.totalSupply = TestTokenA.totalSupply + amount
  ...
  access(all) resource Administrator {
    ...
    // transactions/testTokenA/minter/setup_testTokenA_minter.cdc
    // then the admin account running:
    // transactions/testTokenA/admin/deposit_testTokenA_minter.cdc
    //
  ...
  init() {
    self.AdminStoragePath = /storage/testTokenAAdmin
    self.MinterProxyPublicPath = /public/testTokenAMinterProxy
    self.MinterProxyStoragePath = /storage/testTokenAMinterProxy
