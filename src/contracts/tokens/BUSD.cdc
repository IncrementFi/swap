...
access(all) contract BUSD: FungibleToken {
  ...
  // Total supply of busd in existence
  ...
  access(all) resource Vault: FungibleToken.Vault {
    ...
    access(contract) fun burnCallback() {
      if self.balance > 0.0 {
        BUSD.totalSupply = BUSD.totalSupply - self.balance
      }
      self.balance = 0.0
    }
  ...
  //
  access(all) fun createEmptyVault(vaultType: Type): @BUSD.Vault {
    return <-create Vault(balance: 0.0)
  }
  ...
  access(all) resource Minter {
    ...
    //
    access(all) fun mintTokens(amount: UFix64): @BUSD.Vault {
      pre {
        amount > 0.0: "Amount minted must be greater than zero"
      }
      BUSD.totalSupply = BUSD.totalSupply + amount
  ...
  access(all) resource Administrator {
    ...
    // transactions/busd/minter/setup_busd_minter.cdc
    // then the admin account running:
    // transactions/busd/admin/deposit_busd_minter.cdc
    //
  ...
  init() {
    self.AdminStoragePath = /storage/busdAdmin
    self.MinterProxyPublicPath = /public/busdMinterProxy
    self.MinterProxyStoragePath = /storage/busdMinterProxy
