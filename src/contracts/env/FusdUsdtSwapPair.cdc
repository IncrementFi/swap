import FUSD from "../tokens/FUSD.cdc" // 0x3c5959b568896393
...
access(all) contract FusdUsdtSwapPair: FungibleToken {
  ...
  //
  access(all) fun createEmptyVault(vaultType: Type): @FusdUsdtSwapPair.Vault {
    return <-create Vault(balance: 0.0)
  }
  ...
  access(all) resource TokenBundle {
    access(all) var token1: @FUSD.Vault
    ...
    access(all) fun depositToken1(from: @FUSD.Vault) {
    ...
    access(all) fun withdrawToken1(): @FUSD.Vault {
      var vault <- FUSD.createEmptyVault(vaultType: Type<@FUSD.Vault>())
      vault <-> self.token1
      return <- vault
    }
  ...
  // createEmptyBundle
  //
  access(all) fun createEmptyTokenBundle(): @FusdUsdtSwapPair.TokenBundle {
    return <- create TokenBundle(
      fromToken1: <- (FUSD.createEmptyVault(vaultType: Type<@FUSD.Vault>())),
  ...
  // createTokenBundle
  //
  access(all) fun createTokenBundle(fromToken1: @FUSD.Vault, fromToken2: @TeleportedTetherToken.Vault): @FusdUsdtSwapPair.TokenBundle {
  ...
  access(all) resource SwapProxy {
    access(all) fun swapToken1ForToken2(from: @FUSD.Vault): @TeleportedTetherToken.Vault {
      return <- FusdUsdtSwapPair._swapToken1ForToken2(from: <-from)
    }

    access(all) fun swapToken2ForToken1(from: @TeleportedTetherToken.Vault): @FUSD.Vault {
      return <- FusdUsdtSwapPair._swapToken2ForToken1(from: <-from)
    }

    access(all) fun addLiquidity(from: @FusdUsdtSwapPair.TokenBundle): @FusdUsdtSwapPair.Vault {
      return <- FusdUsdtSwapPair._addLiquidity(from: <-from)
    }

    access(all) fun removeLiquidity(from: @FusdUsdtSwapPair.Vault, token1Amount: UFix64, token2Amount: UFix64): @FusdUsdtSwapPair.TokenBundle {
      return <- FusdUsdtSwapPair._removeLiquidity(from: <-from, token1Amount: token1Amount, token2Amount: token2Amount)
    }
  }

  access(all) resource interface SwapProxyReceiver {
    access(all) fun depositSwapProxy(from: @FusdUsdtSwapPair.SwapProxy)
  }

  access(all) resource SwapProxyHolder: SwapProxyReceiver {
    // holder for SwapProxy resource
    access(all) var swapProxy: @FusdUsdtSwapPair.SwapProxy?
    ...
    access(all) fun withdrawSwapProxy(): @FusdUsdtSwapPair.SwapProxy {
      pre {
  ...
  access(all) fun createSwapProxyHolder(): @FusdUsdtSwapPair.SwapProxyHolder {
  ...
  access(all) resource Admin {
    ...
    access(all) fun setProxyOnly(proxyOnly: Bool) {
    ...
    access(all) fun createSwapProxy(): @FusdUsdtSwapPair.SwapProxy {
  ...
  access(all) view fun proxyOnly(): Bool {
  ...
  access(all) fun donateLiquidity(from: @FusdUsdtSwapPair.TokenBundle) {
  ...
  access(all) fun addInitialLiquidity(from: @FusdUsdtSwapPair.TokenBundle): @FusdUsdtSwapPair.Vault {
    pre {
  ...
  init() {
    ...
    self.token1Vault <- FUSD.createEmptyVault(vaultType: Type<@FUSD.Vault>())
