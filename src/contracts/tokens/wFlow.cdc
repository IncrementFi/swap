...
access(all) contract wFlow: FungibleToken {
  ...
  // Total supply of wflow in existence
  ...
  access(all) resource Vault: FungibleToken.Vault {
    ...
    access(contract) fun burnCallback() {
      if self.balance > 0.0 {
        wFlow.totalSupply = wFlow.totalSupply - self.balance
      }
      self.balance = 0.0
    }
  ...
  //
  access(all) fun createEmptyVault(vaultType: Type): @wFlow.Vault {
    return <-create Vault(balance: 0.0)
  }
  ...
  access(all) resource Minter {
    ...
    //
    access(all) fun mintTokens(amount: UFix64): @wFlow.Vault {
        pre {
            amount > 0.0: "Amount minted must be greater than zero"
        }
        wFlow.totalSupply = wFlow.totalSupply + amount
  ...
  access(all) resource Administrator {
    ...
    // transactions/wflow/minter/setup_wflow_minter.cdc
    // then the admin account running:
    // transactions/wflow/admin/deposit_wflow_minter.cdc
    //
  ...
  init() {
    self.AdminStoragePath = /storage/wflowAdmin
    self.MinterProxyPublicPath = /public/wflowMinterProxy
    self.MinterProxyStoragePath = /storage/wflowMinterProxy
